...
async def get_appl_page(request: Request):
    ...
    try:
        ...
        for row in rows:
            # row: id, header, salary_min, date, description, author_id, photos, responses_count, profession, executor_id
            ...
            if user_responded:
                ...
                vacancies.append({
                    ...
                    "dedline": row[3],
                    "cost": row[2],
                    "otcklick": str(len(responses)),  # общее количество откликов
                })
        return templates.TemplateResponse(request, "applications.html", {"vacancies": vacancies})
...
async def get_task_detail(request: Request, vacancy_id: str):
    ...
    executor_id = vacancy[9]  # теперь сохраняем

    # Получаем имя автора и инициалы
    ...
    # Парсим отклики (массив объектов)
    try:
    ...
    # Находим цену текущего пользователя
    my_cost = None
    ...
    task_data = {
        ...
        "my_cost": my_cost,
        "executor_id": executor_id,  # <-- добавлено
    }
...
async def update_task_cost(request: Request, task_id: str):
    ...
    # Поиск индекса текущего пользователя
    index = None
    ...
    if price == 0:
        ...
        if index is not None:
            # Обновляем только цену, инициалы оставляем
            responses[index]["cost"] = price
        else:
            # Получаем имя пользователя и вычисляем инициалы
...
from fastapi.staticfiles import StaticFiles
from fastapi.templating import Jinja2Templates
import os, random, uvicorn, uuid, asyncio, aiosmtplib, re, shutil, json
from email.message import EmailMessage
from typing import Optional, List
from requestBD import request_bd, init_db
